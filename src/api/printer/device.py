...
class UsbDevice(DeviceImpl):
    ...
    async def _read(cls, size):
        ...
        try:
            output =await loop.run_in_executor(None, partial(cls.device.read,cls.IN_EP, size)) #type: ignore
    ...
    async def _write(cls, data):
        ...
        loop = asyncio.get_running_loop()
        try:
            await loop.run_in_executor(None, partial(cls.device.write,cls.OUT_EP, data)) #type:ignore
        except (usb.core.USBError, usb.core.USBTimeoutError, IOError) as e:
            raise DeviceIOError(e)
